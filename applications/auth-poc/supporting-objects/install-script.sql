...
--
-- Re-runs / upgrades:
--   • Tables/indexes are plain CREATE → ORA-00955 if already present is normal (skip those lines).
--     Oracle has no CREATE OR REPLACE TABLE; use DDL only on first install, or DROP (careful) first.
--   • PACKAGE / PACKAGE BODY use CREATE OR REPLACE — safe to rerun as often as needed.
--   • For recurring runs when schema already has tables: prefer supporting-objects/install-package-only.sql
-- XX_AUTH_PKG uses STANDARD_HASH (SHA512) and SYS_GUID only — no DBMS_CRYPTO.
-- Suitable for workspaces where EXECUTE ON SYS.DBMS_CRYPTO is not granted.
--
-- ── XX_USERS ──
