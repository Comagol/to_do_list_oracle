-- ============================================================
-- Auth Multi-Scheme POC — Supporting Objects Install Script
-- Tables, indexes, PL/SQL package, seed data
-- ============================================================
...
CREATE TABLE xx_users (
    ...
    birth_date            DATE,
    is_active             VARCHAR2(1)    DEFAULT 'Y'  NOT NULL CONSTRAINT xx_users_active_ck  CHECK (is_active  IN ('Y','N')),
    is_locked             VARCHAR2(1)    DEFAULT 'N'  NOT NULL CONSTRAINT xx_users_locked_ck  CHECK (is_locked  IN ('Y','N')),
    ...
    sso_provider          VARCHAR2(100),
    sso_subject_id        VARCHAR2(500),
    force_password_change VARCHAR2(1)    DEFAULT 'N' CONSTRAINT xx_users_fpc_ck CHECK (force_password_change IN ('Y','N')),
    created_at            TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    created_by            VARCHAR2(100),
    updated_at            TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    updated_by            VARCHAR2(100),
    CONSTRAINT xx_users_username_uk UNIQUE (username),
    CONSTRAINT xx_users_email_uk   UNIQUE (email)
);
/

CREATE INDEX xx_users_sso_idx ON xx_users (sso_provider, sso_subject_id);
/

-- ── XX_USER_PASSWORDS ──
CREATE TABLE xx_user_passwords (
    id              NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id         NUMBER         NOT NULL REFERENCES xx_users(id),
    password_hash   VARCHAR2(512)  NOT NULL,
    password_salt   VARCHAR2(256)  NOT NULL,
    is_active       VARCHAR2(1)    DEFAULT 'Y' NOT NULL CONSTRAINT xx_user_pwd_active_ck CHECK (is_active IN ('Y','N')),
    created_at      TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    created_by      VARCHAR2(100)
);
/

CREATE INDEX xx_user_pwd_user_idx   ON xx_user_passwords (user_id);
CREATE INDEX xx_user_pwd_active_idx ON xx_user_passwords (user_id, is_active);
/

-- ── XX_USER_ACCESS_REQUEST ──
CREATE TABLE xx_user_access_request (
    id              NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    sso_provider    VARCHAR2(100),
    sso_subject_id  VARCHAR2(500),
    email           VARCHAR2(255)  NOT NULL,
    first_name      VARCHAR2(100),
    last_name       VARCHAR2(100),
    status          VARCHAR2(20)   DEFAULT 'PENDING' NOT NULL CONSTRAINT xx_uar_status_ck CHECK (status IN ('PENDING','APPROVED','REJECTED')),
...
CREATE INDEX xx_uar_status_idx ON xx_user_access_request (status);
CREATE INDEX xx_uar_email_idx  ON xx_user_access_request (email);
/

-- ── XX_LOGIN_AUDIT ──
CREATE TABLE xx_login_audit (
    id                 NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id            NUMBER REFERENCES xx_users(id),
    username_attempted VARCHAR2(100) NOT NULL,
    login_type         VARCHAR2(20)  NOT NULL CONSTRAINT xx_la_type_ck    CHECK (login_type IN ('PASSWORD','SSO')),
    success            VARCHAR2(1)   NOT NULL CONSTRAINT xx_la_success_ck CHECK (success    IN ('Y','N')),
...
CREATE INDEX xx_la_user_idx ON xx_login_audit (user_id);
CREATE INDEX xx_la_ts_idx   ON xx_login_audit (created_at);
/

-- ── XX_PASSWORD_RESET_TOKENS ──
CREATE TABLE xx_password_reset_tokens (
    id          NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id     NUMBER         NOT NULL REFERENCES xx_users(id),
    token_hash  VARCHAR2(512)  NOT NULL,
    expires_at  TIMESTAMP      NOT NULL,
    is_used     VARCHAR2(1)    DEFAULT 'N' NOT NULL CONSTRAINT xx_prt_used_ck CHECK (is_used IN ('Y','N')),
    used_at     TIMESTAMP,
    created_at  TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL
);
/

CREATE INDEX xx_prt_user_idx ON xx_password_reset_tokens (user_id);
CREATE INDEX xx_prt_hash_idx ON xx_password_reset_tokens (token_hash);
/

-- ── XX_APP_PARAMETERS ──
CREATE TABLE xx_app_parameters (
    id               NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    updated_at       TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
    updated_by       VARCHAR2(100),
    CONSTRAINT xx_app_params_name_uk UNIQUE (parameter_name)
);
/
...
CREATE OR REPLACE PACKAGE BODY xx_auth_pkg AS
    ...
    BEGIN
        RETURN rawtohex(dbms_crypto.hash(utl_raw.cast_to_raw(p_salt || p_password), dbms_crypto.hash_sh256));
    END hash_password;
    ...
    FUNCTION generate_reset_token (p_user_id IN NUMBER) RETURN VARCHAR2 IS
        v_token VARCHAR2(128); v_hash VARCHAR2(512); v_min NUMBER;
    BEGIN
        v_token := rawtohex(dbms_crypto.randombytes(64));
        v_hash  := rawtohex(dbms_crypto.hash(utl_raw.cast_to_raw(v_token), dbms_crypto.hash_sh256));
    ...
    BEGIN
        v_hash := rawtohex(dbms_crypto.hash(utl_raw.cast_to_raw(p_token), dbms_crypto.hash_sh256));
...
-- ── Seed parameters ──
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('FORGET_PASSWORD_TOKEN_EXPIRATION', '60', 'Token expiration in minutes for the forgot-password flow', 'SYSTEM', 'SYSTEM');
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('MAX_FAILED_LOGIN_ATTEMPTS', '5', 'Maximum consecutive failed logins before account lockout', 'SYSTEM', 'SYSTEM');
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('LOCKOUT_DURATION_MINUTES', '30', 'Duration in minutes an account stays locked after max failures', 'SYSTEM', 'SYSTEM');
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('PASSWORD_EXPIRATION_DAYS', '90', 'Days until a password expires and must be changed', 'SYSTEM', 'SYSTEM');
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('PASSWORD_HISTORY_COUNT', '5', 'Number of previous passwords checked to prevent reuse', 'SYSTEM', 'SYSTEM');
INSERT INTO xx_app_parameters (parameter_name, parameter_value, description, created_by, updated_by) VALUES ('RESET_TOKEN_LENGTH', '64', 'Byte length of generated password-reset tokens', 'SYSTEM', 'SYSTEM');
COMMIT;
/
